...
//      SYSCTL_RCGCGPIO_R |= 0x02; //puts portB in run mode
//	    while ((SYSCTL_PRGPIO_R & 0XFFFFFFC2) == 0) {};
//		  GPIO_PORTB_DEN_R |= 0x03;
//		  GPIO_PORTB_AFSEL_R |= 0x03;
//      GPIO_PORTB_PCTL_R &= 0x0FFFFFF00;     // Force 0's in the desired locations
...
//	   cyBOT_init_Scan();
	...
	while(1)
	{
	    ...
	    //for sendStr, initialize str with a while loop, adding a new c to every index

	    c = uart_receive();
	    lcd_putc(c); //prints to lcd
        uart_sendChar(c);
